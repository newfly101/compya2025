...
export const pickSkillsByCombo = (skills, combo) => {
  const legendPool = [...skills.legend];
  const platinumPool = [...skills.platinum];
  const normalPool = [
    ...skills.normal.map(skill => ({
      ...skill,
      tier: "NORMAL"
    })),
    ...skills.hero.map(skill => ({
      ...skill,
      tier: "HERO"
    }))
  ];
