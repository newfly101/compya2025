 ...
 * legendStat / mileage 도메인의 Snapshot 과 동일한 패턴.
 */
public record PlayerCardSnapshot<T>(List<T> items, String etag) {

    /**
     * ETag 는 반드시 weak(W/) 여야 한다.
     * Tomcat 은 strong ETag 가 붙은 응답을 gzip 하지 않는다 — 304 동작은 동일하다.
     */
    public static <T> PlayerCardSnapshot<T> of(List<T> items) {
        return new PlayerCardSnapshot<>(items, "W/\"" + hash(items.toString()) + "\"");
    }
